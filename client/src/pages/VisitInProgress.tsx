...
export default function VisitInProgress() {
  ...
  const updateChecklistMutation = useMutation({
    ...
    onError: (_err, { itemId, isCompleted }) => {
      // Revert local state if the server save failed
      setChecklist((prev) => ({ ...prev, [itemId]: !isCompleted }));
      toast({ title: "Error al guardar ítem", description: "No se pudo guardar el cambio, intenta de nuevo", variant: "destructive" });
    },
  });
  ...
  const completeVisitMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "No se pudo completar la visita",
        description: error?.message || "Verifica que todos los ítems del checklist estén completados",
        variant: "destructive",
      });
    },
  });
        ...
        {totalItems > 0 && completedItems < totalItems && (
          <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-950/30 border-b border-amber-200 dark:border-amber-800">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-300">
              Faltan <strong>{totalItems - completedItems}</strong> ítem{totalItems - completedItems !== 1 ? "s" : ""} del checklist por completar antes de cerrar
            </p>
          </div>
        )}
        <div className="p-4">
